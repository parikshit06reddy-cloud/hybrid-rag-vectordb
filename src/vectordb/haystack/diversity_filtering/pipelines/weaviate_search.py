"""Weaviate search pipeline with diversity filtering and RAG.

Implements search with Maximum Margin Relevance (MMR) diversity filtering
for Weaviate vector database, with optional RAG answer generation.

Pipeline Flow:
1. Embed query using SentenceTransformersTextEmbedder
2. Retrieve top_k_candidates from Weaviate using vector similarity
...
Configuration:
Pipeline behavior controlled via YAML config with Weaviate connection params,
retrieval settings, diversity algorithm options, and RAG settings.
"""
...
def run_search(config_path: str, query: str) -> dict:
    """Run Weaviate search pipeline with diversity filtering and optional RAG.
