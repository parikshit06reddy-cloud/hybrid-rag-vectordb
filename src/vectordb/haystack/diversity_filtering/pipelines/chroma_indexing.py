"""Chroma indexing pipeline for diversity filtering feature.

Indexes documents from standard datasets into Chroma for diversity-aware
...
Supports both ephemeral (in-memory) and persistent storage modes configured
via is_persistent flag. Documents are indexed with full embedding vectors.
"""
...
def run_indexing(config_path: str) -> dict:
    ...
    db = ChromaVectorDB(
        ...
        port=config.vectordb.chroma.port,
        index=config.index.name,
        embedding_dim=config.embedding.dimension,
        is_persistent=config.vectordb.chroma.is_persistent,
    )
