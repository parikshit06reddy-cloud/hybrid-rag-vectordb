"""Milvus search pipeline with diversity filtering and RAG.

Implements search with Maximum Margin Relevance (MMR) diversity filtering
for Milvus vector database, with optional RAG answer generation.

Pipeline Flow:
1. Embed query using SentenceTransformersTextEmbedder
2. Retrieve top_k_candidates from Milvus using vector similarity
...
Configuration:
Pipeline behavior controlled via YAML config with Milvus connection params
(host, port, db_name), retrieval settings, diversity algorithm options,
and RAG settings.
"""
...
def run_search(config_path: str, query: str) -> dict:
    """Run Milvus search pipeline with diversity filtering and optional RAG.
    ...
    db = MilvusVectorDB(
        host=config.vectordb.milvus.host,
        port=config.vectordb.milvus.port,
        db_name=config.vectordb.milvus.db_name,
