"""Pinecone JSON indexing pipeline for Haystack."""
...
class PineconeJSONIndexer:
    """Indexes documents into Pinecone with JSON metadata.

    Attributes:
        config: Configuration dictionary.
        logger: Logger instance.
        vector_db: PineconeVectorDB wrapper instance.
    ...
    def _setup_logging(self) -> None:
        ...
        name = logging_config.get("name", "pinecone_json_indexing")
    ...
    def _connect(self) -> None:
        """Connect to Pinecone using VectorDB wrapper."""
    ...
    def run(self) -> dict[str, Any]:
        ...
        # Upsert to Pinecone (creates index if needed)
        self.vector_db.upsert_documents(
            embedded_docs,
            namespace=self.config.get("pinecone", {}).get("namespace"),
        )
