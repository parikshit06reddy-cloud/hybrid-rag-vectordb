"""Qdrant multi-tenancy search pipeline with RAG support."""
...
class QdrantMultitenancySearchPipeline:
    """Qdrant search pipeline with payload-based tenant isolation.
    ...
    def __init__(
        ...
        """Initialize Qdrant search pipeline.
    ...
    def _connect(self) -> None:
        """Connect to Qdrant and initialize components."""
    ...
    def query(
        ...
        with Timer() as timer:
            ...
            tenant_filter = Filter(
                must=[
                    FieldCondition(
                        key=self.TENANT_FIELD, match=MatchValue(value=target_tenant)
                    )
                ]
            )
            ...
            for result in results:
                ...
                doc = Document(content=content, meta=result.payload or {})
