"""Milvus multi-tenancy search pipeline with RAG support."""
...
class MilvusMultitenancySearchPipeline:
    """Milvus search pipeline with partition key tenant isolation.
    ...
    def __init__(
        ...
        """Initialize Milvus search pipeline.
    ...
    def _connect(self) -> None:
        """Connect to Milvus and initialize components."""
    ...
    def query(
        ...
        for i, doc in enumerate(results):
            query_result = TenantQueryResult(
                document=doc,
                relevance_score=1.0,  # Placeholder - actual score would come from DB
                rank=i + 1,
        ...
        documents = [qr.document for qr in query_results]
        scores = [qr.relevance_score for qr in query_results]
