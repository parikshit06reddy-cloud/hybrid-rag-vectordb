...
def main():
    ...
    parser.add_argument("--collection_name1", type=str, required=True, help="Name of the first Chroma collection.")
    parser.add_argument("--collection_name2", type=str, required=True, help="Name of the second Chroma collection.")

    # Embedding model arguments
    parser.add_argument("--dense_model", type=str, required=True, help="Dense embedding model name.")

    parser.add_argument("--dataset_name", required=True, help="Name of the dataset to be used by the dataloader.")
    parser.add_argument("--split1", required=True, help="Dataset split for the first collection.")
    parser.add_argument("--split2", required=True, help="Dataset split for the second collection.")
    parser.add_argument("--tracing_project_name", default="chroma", help="Name of the Weave project for tracing.")
    parser.add_argument("--weave_params", type=str, help="JSON string of parameters for initializing Weave.")
    ...
    if args.generator_model and args.generator_api_key:
        generator_params = eval(args.generator_llm_params) if args.generator_llm_params else {}
        generator = ChatGroqGenerator(
            model=args.generator_model,
            api_key=args.generator_api_key,
            llm_params=generator_params,
    ...
    dataloader_split1 = dataloader_cls(
        dataset_name=args.dataset_name, split=args.split1, answer_summary_generator=generator
    )
    dataloader_split2 = dataloader_cls(
        dataset_name=args.dataset_name, split=args.split2, answer_summary_generator=generator
    )
    ...
    docs_with_embeddings_split1 = embedder.run(documents=haystack_documents_split1)["documents"]
    docs_with_embeddings_split2 = embedder.run(documents=haystack_documents_split2)["documents"]
    ...
    docs_for_chroma_split1 = ChromaDocumentConverter.prepare_haystack_documents_for_upsert(docs_with_embeddings_split1)
    docs_for_chroma_split2 = ChromaDocumentConverter.prepare_haystack_documents_for_upsert(docs_with_embeddings_split2)
    ...
    chroma_vector_db.upsert(data=docs_for_chroma_split1, collection_name=args.collection_name1)

    chroma_vector_db.create_collection(name=args.collection_name2)
    chroma_vector_db.upsert(data=docs_for_chroma_split2, collection_name=args.collection_name2)
    ...
    retrieval_results_split1 = ChromaDocumentConverter.convert_query_results_to_haystack_documents(
    ...
    retrieval_results_split2 = ChromaDocumentConverter.convert_query_results_to_haystack_documents(
