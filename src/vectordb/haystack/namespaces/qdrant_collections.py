...
def main():
    ...
    parser.add_argument(
        "--qdrant_api_key", required=True, help="API key for accessing Qdrant."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--query", type=str, required=True, help="Query string for Qdrant search."
    )
    ...
    qdrant_client.create_collection(
        ...
        vector_size=docs_with_embeddings[0]["embedding"].shape[
            0
        ],  # Assuming all embeddings are of the same size
        distance="Cosine",
    )
    qdrant_client.create_collection(
    ...
    docs_for_qdrant = QdrantDocumentConverter.prepare_haystack_documents_for_upsert(
        docs_with_embeddings
    )

    # Upsert data into Qdrant collections
    qdrant_client.upsert(collection_name=args.collection_name1, points=docs_for_qdrant)
    qdrant_client.upsert(collection_name=args.collection_name2, points=docs_for_qdrant)
