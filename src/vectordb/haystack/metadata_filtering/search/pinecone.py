"""Pinecone metadata filtering search pipeline for Haystack.

Performs vector search with metadata filtering on Pinecone.
"""
...
__all__ = ["PineconeMetadataFilteringSearchPipeline"]

logger = logging.getLogger(__name__)


class PineconeMetadataFilteringSearchPipeline:
    """Pinecone metadata filtering search pipeline.
    ...
    def __init__(self, config_or_path: str | dict) -> None:
        """Initialize Pinecone search pipeline from configuration.
    ...
    def _validate_config(self) -> None:
        ...
        required_sections = ["embeddings", "pinecone", "search"]
    ...
    def search(self, query: str | None = None) -> list[FilteredQueryResult]:
        ...
        timing = TimingMetrics(
            pre_filter_ms=0.0,  # Pinecone applies filtering during query
            vector_search_ms=search_timer.elapsed_ms,
            total_ms=search_timer.elapsed_ms,
            num_candidates=-1,  # Not easily retrievable
            num_total_docs=-1,
        )
