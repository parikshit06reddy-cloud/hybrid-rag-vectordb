...
def main():
    parser = argparse.ArgumentParser(description="Query Chroma vector database with a dense embedding.")
    ...
    # Embedder parameters
    parser.add_argument("--embedding_model",type = str, help="Model to use for generating document embeddings.")
    parser.add_argument("--embedding_model_params", type=str, help="JSON string of parameters for the embedding model.")


    # Query arguments
    parser.add_argument("--question", required=True, help="Question to query the Chroma database.")
    parser.add_argument("--query_filter", type=str, help="JSON string for query filter.")
    ...
    query_response = chroma_vector_db.query(
        query_embedding=dense_question_embedding, n_results=args.n_results, where_document=query_filter
    )
