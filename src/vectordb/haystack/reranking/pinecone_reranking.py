...
def main():
    """Perform hybrid embedding, retrieval, and reranking using Pinecone."""
    ...
    embedder.warm_up()
    embedder.run(documents=haystack_documents)["documents"]
    ...
    # Rerank results using Pinecone's rerank model
    reranked_result = pinecone_vector_db.client.inference.rerank(
        ...
        parameters=eval(args.rerank_params),
    )
