...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Dimensionality of the sparse vectors.",
    )
    parser.add_argument(
        "--metric_type",
        default="IP",
        help="Metric type for similarity search (default: 'IP').",
    )

    # Sparse Index parameters
    parser.add_argument(
        "--index_name", default="sparse_index", help="Name of the sparse index."
    )
    parser.add_argument(
        "--drop_ratio_build",
        type=float,
        default=0.2,
        help="Drop ratio for small vector values.",
    )
    ...
    print(
        f"Indexed {len(sparse_embeddings)} sparse vectors into the collection: {args.collection_name}"
    )
