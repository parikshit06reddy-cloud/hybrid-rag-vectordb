"""Milvus Parent Document Search Pipeline.

This pipeline implements parent document retrieval by searching child chunks
in Milvus and resolving matches to their parent documents.

Parent Document Retrieval Strategy:
    1. Embed the search query using the same model as indexing
    2. Search child chunks in Milvus vector database (oversample for better recall)
...
Search Flow:
    ...
    2. Query Milvus for top child matches (top_k * 3 for oversampling)
...
Configuration (YAML):
    Required sections:
        - database.milvus: Milvus connection and collection settings
...
Usage:
    ...
    >>> indexing = MilvusParentDocIndexingPipeline("config.yaml")
    >>> stats = indexing.run(limit=100)
    >>> # Then, search
    >>> search = MilvusParentDocSearchPipeline(
...
Attributes:
    ...
    vector_db: MilvusVectorDB connection for searching leaves
    ...
    index_name: Name of the Milvus collection for leaves
"""
...
class MilvusParentDocSearchPipeline:
    """Milvus search pipeline for parent document retrieval.

    This pipeline searches child chunks in Milvus and uses the parent_id
    ...
    2. Search Milvus for matching child chunks
    ...
    Attributes:
        ...
        parent_store: InMemoryDocumentStore containing parent documents
        auto_merger: AutoMergingRetriever for resolving children to parents
        index_name: Name of the Milvus collection
    """
    ...
    def __init__(
        ...
        Loads configuration, initializes the query embedder, Milvus connection,
    ...
    def _init_vector_db(self) -> None:
        """Initialize the Milvus vector database connection.

        Creates a MilvusVectorDB instance connected to the same collection
        used during indexing. This enables searching the child chunks that
        were indexed.

        Configuration:
            database.milvus: Milvus connection settings
            database.milvus.collection_name: Collection name
                (default: "parent_doc_leaves")
        """
        db_config = self.config.get("database", {})
        self.vector_db = MilvusVectorDB(config={"milvus": db_config.get("milvus", {})})
        self.index_name = db_config.get("milvus", {}).get(
            "collection_name", "parent_doc_leaves"
        )
    ...
    def search(self, query: str, top_k: int = 5) -> dict:
        ...
        searches child chunks in Milvus, and resolves matches to parent
        ...
        Search Strategy:
            1. Embed the query using SentenceTransformers
            2. Search Milvus for top_k * 3 child matches (oversample for recall)
            3. Use AutoMergingRetriever to resolve children to parents
