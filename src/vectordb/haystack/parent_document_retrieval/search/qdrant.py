"""Qdrant Parent Document Search Pipeline.

This pipeline implements parent document retrieval by searching child chunks
in Qdrant and resolving matches to their parent documents.

Parent Document Retrieval Strategy:
    1. Embed the search query using the same model as indexing
    2. Search child chunks in Qdrant vector database (oversample for better recall)
...
Configuration (YAML):
    Required sections:
        - database.qdrant: Qdrant connection and collection settings
...
Usage:
    ...
    >>> indexing = QdrantParentDocIndexingPipeline("config.yaml")
    >>> stats = indexing.run(limit=100)
    >>> # Then, search
    >>> search = QdrantParentDocSearchPipeline(
...
Attributes:
    ...
    vector_db: QdrantVectorDB connection for searching leaves
    ...
    index_name: Name of the Qdrant collection for leaves
"""
...
class QdrantParentDocSearchPipeline:
    """Qdrant search pipeline for parent document retrieval.

    This pipeline searches child chunks in Qdrant and uses the parent_id
    ...
    2. Search Qdrant for matching child chunks
    ...
    def __init__(
        ...
        Loads configuration, initializes the query embedder, Qdrant connection,
    ...
    def _init_vector_db(self) -> None:
        ...
        Creates a QdrantVectorDB instance connected to the same collection
        used during indexing. This enables searching the child chunks that
        were indexed.

        Configuration:
            database.qdrant: Qdrant connection settings
            database.qdrant.collection_name: Collection name
                (default: "parent_doc_leaves")
        """
        db_config = self.config.get("database", {})
        self.vector_db = QdrantVectorDB(config={"qdrant": db_config.get("qdrant", {})})
        self.index_name = db_config.get("qdrant", {}).get(
            "collection_name", "parent_doc_leaves"
        )
    ...
    def search(self, query: str, top_k: int = 5) -> dict:
        ...
        searches child chunks in Qdrant, and resolves matches to parent
        ...
        Search Strategy:
            1. Embed the query using SentenceTransformers
            2. Search Qdrant for top_k * 3 child matches (oversample for recall)
            3. Use AutoMergingRetriever to resolve children to parents
