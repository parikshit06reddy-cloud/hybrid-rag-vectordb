"""Pinecone Parent Document Search Pipeline.

This pipeline implements parent document retrieval by searching child chunks
in Pinecone and resolving matches to their parent documents.

Parent Document Retrieval Strategy:
    1. Embed the search query using the same model as indexing
    2. Search child chunks in Pinecone vector database (oversample for better recall)
...
Search Flow:
    ...
    2. Query Pinecone for top child matches (top_k * 3 for oversampling)
...
Usage:
    ...
    >>> indexing = PineconeParentDocIndexingPipeline("config.yaml")
...
Attributes:
    ...
    vector_db: PineconeVectorDB connection for searching leaves
...
class PineconeParentDocSearchPipeline:
    """Pinecone search pipeline for parent document retrieval.

    This pipeline searches child chunks in Pinecone and uses the parent_id
    ...
    2. Search Pinecone for matching child chunks
    ...
    def __init__(
        ...
        Loads configuration, initializes the query embedder, Pinecone connection,
    ...
    def _init_vector_db(self) -> None:
        ...
        Creates a PineconeVectorDB instance connected to the same index
        ...
        self.index_name = db_config.get("pinecone", {}).get(
            "index_name", "parent-doc-leaves"
        )
    ...
    def search(self, query: str, top_k: int = 5) -> dict:
        ...
        searches child chunks in Pinecone, and resolves matches to parent
        ...
        Search Strategy:
            1. Embed the query using SentenceTransformers
            2. Search Pinecone for top_k * 3 child matches (oversample for recall)
            3. Use AutoMergingRetriever to resolve children to parents
        ...
        # Search for matching child chunks in Pinecone
