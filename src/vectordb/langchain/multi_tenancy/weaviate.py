"""Weaviate multi-tenancy implementation using collections.

This module implements the MultiTenancyPipeline interface for Weaviate,
using Weaviate's collection mechanism for tenant isolation.
"""
...
class WeaviateMultiTenancyPipeline(MultiTenancyPipeline):
    """Weaviate multi-tenancy pipeline using collections for isolation.

    Each tenant's data is stored in a separate Weaviate collection,
    ...
    def __init__(
        ...
        """Initialize Weaviate multi-tenancy pipeline.
        ...
        logger.info(
            "Initialized Weaviate multi-tenancy pipeline at URL: %s",
            url,
        )
    ...
    def search_for_tenant(
        ...
        Args:
            ...
            query_embedding: Pre-computed query embedding vector. Required for Weaviate
        ...
        if query_embedding is None:
            raise ValueError(
                "query_embedding is required for Weaviate search. "
    ...
    def list_tenants(self) -> list[str]:
        """List all active tenants (collections) in Weaviate.
        ...
        try:
            ...
            logger.info("Found %d tenants in Weaviate", len(tenants))
            return tenants
        except Exception as e:
