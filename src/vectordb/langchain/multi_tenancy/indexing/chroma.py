"""Chroma multi-tenancy indexing pipeline (LangChain).
...
different tenants (customers, organizations, users) using Chroma's collection
mechanism.

Multi-Tenancy Strategy:
    Chroma collections provide perfect tenant isolation:
    ...
    - Query performance: Collection filtering is efficient in Chroma
...
See Also:
    - vectordb.langchain.multi_tenancy.search.chroma: Tenant-scoped search
...
class ChromaMultiTenancyIndexingPipeline:
    """Chroma indexing pipeline for multi-tenant scenarios (LangChain).

        Indexes documents for a specific tenant in an isolated collection,
        ensuring complete data segregation between tenants. Uses Chroma's
    ...
    def __init__(self, config_or_path: dict[str, Any] | str, tenant_id: str) -> None:
        ...
        Args:
            ...
            tenant_id: Unique identifier for the tenant. This will be used as
                the collection name in Chroma. Cannot be empty.
        ...
        logger.info(
            "Initialized Chroma multi-tenancy indexing pipeline for tenant: %s",
            tenant_id,
        )
    ...
    def run(self) -> dict[str, Any]:
        ...
        logger.info(
            "Indexed %d documents for tenant %s to Chroma",
            num_indexed,
            self.tenant_id,
        )
