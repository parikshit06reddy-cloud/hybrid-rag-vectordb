"""Weaviate multi-tenancy search pipeline (LangChain).
...
Tenant Isolation Model:
    Each tenant's data is stored in a Weaviate collection named by tenant_id.
...
Search Flow:
    ...
    6. Query Weaviate within tenant's collection only
...
See Also:
    - vectordb.langchain.multi_tenancy.indexing.weaviate: Tenant-scoped indexing
...
class WeaviateMultiTenancySearchPipeline:
    """Weaviate search pipeline for multi-tenant scenarios (LangChain).
    ...
    Design Decisions:
        - Collection isolation: Uses Weaviate collections for tenant isolation,
    ...
    Example:
        ...
        ... }
        >>> pipeline = WeaviateMultiTenancySearchPipeline(config, tenant_id="tenant_1")
    ...
    def __init__(self, config_or_path: dict[str, Any] | str, tenant_id: str) -> None:
        ...
        logger.info(
            "Initialized Weaviate multi-tenancy search pipeline for tenant: %s",
            tenant_id,
        )
    ...
    def search(
        ...
        The search process:
            ...
            2. Query Weaviate within tenant's collection only
        ...
        # Query Weaviate within tenant-specific collection only.
        ...
        logger.info(
            "Retrieved %d documents for tenant %s from Weaviate",
            len(documents),
            self.tenant_id,
        )
