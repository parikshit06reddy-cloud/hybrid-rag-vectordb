"""Pinecone multi-tenancy search pipeline (LangChain).
...
namespace, ensuring complete data isolation between tenants.

Tenant Isolation Model:
    Each tenant's data is stored in a Pinecone namespace named by tenant_id.
    This search pipeline:
    - Validates tenant_id at initialization
    - Embeds the query using the configured embedder
    - Queries ONLY within the tenant's namespace
...
Search Flow:
    ...
    6. Query Pinecone within tenant's namespace only
...
Security Guarantees:
    - Search is scoped to namespace=tenant_id
...
See Also:
    - vectordb.langchain.multi_tenancy.indexing.pinecone: Tenant-scoped indexing
...
class PineconeMultiTenancySearchPipeline:
    """Pinecone search pipeline for multi-tenant scenarios (LangChain).

    Performs semantic search within a single tenant's isolated namespace,
    ...
    Design Decisions:
        - Namespace isolation: Uses Pinecone namespaces for tenant isolation,
    ...
    Example:
        ...
        ... }
        >>> pipeline = PineconeMultiTenancySearchPipeline(config, tenant_id="tenant_1")
    ...
    def __init__(self, config_or_path: dict[str, Any] | str, tenant_id: str) -> None:
        ...
        Args:
            ...
            tenant_id: Unique identifier for the tenant. Search will be scoped
                to this tenant's namespace. Cannot be empty.
        ...
        Note:
            The tenant_id must match the namespace used during indexing for
        ...
        # Store settings for search operations.
        self.index_name = pinecone_config.get("index_name")
        self.llm = RAGHelper.create_llm(self.config)

        logger.info(
            "Initialized Pinecone multi-tenancy search pipeline for tenant: %s",
            tenant_id,
        )
    ...
    def search(
        ...
        """Execute semantic search within the tenant's isolated namespace.

        Embeds the query and retrieves documents exclusively from the tenant's
        namespace. This ensures complete data isolation - results contain only
        documents belonging to this tenant.

        The search process:
            1. Embed query text using configured embedder
            2. Query Pinecone within tenant's namespace only
        ...
        Args:
            ...
            filters: Optional metadata filters to apply during retrieval.
                Filters are scoped to the tenant's namespace only.
        ...
        # Query Pinecone within tenant-specific namespace only.
        # This is the key isolation mechanism - namespace=self.tenant_id
        ...
        logger.info(
            "Retrieved %d documents for tenant %s from Pinecone",
            len(documents),
            self.tenant_id,
        )
