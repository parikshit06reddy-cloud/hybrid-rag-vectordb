...
def parse_args():
    parser = argparse.ArgumentParser(description="Run a query using Weaviate VectorDB and HuggingFace embeddings.")
    ...
    parser.add_argument(
        "--collection_name", type=str, default="test_collection_dense1", help="Collection name in Weaviate."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--sparse_embedding_model", type=str, default="prithivida/Splade_PP_en_v1", help="Sparse embedding model name."
    )
    parser.add_argument("--question", type=str, required=True, help="The question for which to retrieve answers.")
    parser.add_argument("--top_k", type=int, default=10, help="Number of top results to retrieve.")
    parser.add_argument("--namespace", type=str, default="test_namespace", help="Namespace for Pinecone query.")
