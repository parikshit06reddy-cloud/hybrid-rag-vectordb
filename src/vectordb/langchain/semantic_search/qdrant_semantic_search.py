...
def main():
    ...
    parser.add_argument(
        "--qdrant_host", type=str, required=True, help="Qdrant host URL."
    )
    parser.add_argument(
        "--qdrant_api_key", type=str, required=True, help="API key for Qdrant access."
    )
    parser.add_argument(
        "--index_name", required=True, help="Name of the collection in Qdrant."
    )
    parser.add_argument(
        "--dimension",
        type=int,
    ...
    # Embed the query
    text_embedder.embed_query(args.question)
    sparse_text_embedder.embed_query(args.question)
    ...
    retrieval_results = (
        QdrantDocumentConverter.convert_query_results_to_langchain_documents(
            query_response
        )
    )
