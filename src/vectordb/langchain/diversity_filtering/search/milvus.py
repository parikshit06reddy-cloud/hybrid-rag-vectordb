"""Milvus diversity filtering search pipeline (LangChain).
...
Configuration Schema:
    Required:
        ...
        milvus.collection_name: Target collection name
    Optional:
...
Example:
    ...
    ... )
    >>> pipeline = MilvusDiversityFilteringSearchPipeline("config.yaml")
...
class MilvusDiversityFilteringSearchPipeline:
    """Milvus search pipeline with diversity filtering (LangChain).
    ...
    def __init__(self, config_or_path: dict[str, Any] | str) -> None:
        """Initialize the Milvus diversity filtering search pipeline.

        Loads configuration, initializes the embedder, establishes connection
        to Milvus, and configures diversity filtering parameters.

        Args:
            config_or_path: Configuration dictionary or path to YAML config file.
                Must contain 'milvus' section with host and collection details.

        Raises:
            ValueError: If required Milvus configuration (host) is missing.
            FileNotFoundError: If config_or_path is a file path that doesn't exist.
            ConnectionError: If unable to connect to Milvus API.
        """
        ...
        # Store Milvus settings for search operations.
        ...
        logger.info(
            "Initialized Milvus diversity filtering search pipeline (LangChain)"
        )
