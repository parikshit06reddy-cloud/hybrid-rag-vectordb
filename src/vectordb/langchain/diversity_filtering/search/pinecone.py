"""Pinecone diversity filtering search pipeline (LangChain).
...
Pipeline Architecture:
    ...
    2. Over-fetch: Retrieve 3x top_k candidates from Pinecone
...
See Also:
    - vectordb.langchain.diversity_filtering.indexing.pinecone: Document indexing
...
class PineconeDiversityFilteringSearchPipeline:
    """Pinecone search pipeline with diversity filtering (LangChain).

    Implements diversity-aware document retrieval by over-fetching candidates
    from Pinecone and applying post-processing to select diverse results.
    ...
    def __init__(self, config_or_path: dict[str, Any] | str) -> None:
        """Initialize the Pinecone diversity filtering search pipeline.

        Loads configuration, initializes the embedder, establishes connection
        to Pinecone, and configures diversity filtering parameters.
        ...
        logger.info(
            "Initialized Pinecone diversity filtering search pipeline (LangChain)"
        )
    ...
    def search(
        ...
        Over-fetches candidates from Pinecone, then applies either MMR or
        clustering-based diversity selection to return diverse results.

        The diversity filtering process:
            1. Retrieve 3x top_k candidates from Pinecone
