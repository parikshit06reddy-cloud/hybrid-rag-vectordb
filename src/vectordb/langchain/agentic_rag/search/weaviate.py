"""Weaviate agentic RAG search pipeline (LangChain).

This module implements the search phase of agentic RAG using Weaviate as the
AI-native vector store backend. Weaviate's modular architecture and GraphQL
interface enable sophisticated agentic RAG workflows.

Agentic RAG with Weaviate:
    Weaviate's features enhance agentic RAG capabilities:
    - Hybrid search (vector + BM25) improves retrieval for keyword-heavy queries
    - GraphQL interface allows complex queries with filtering and aggregation
    - Reference properties enable document relationship traversal
    - Native multi-modal support for diverse content types

Weaviate-Specific Considerations:
    - Schema-first approach with property definitions
    - Hybrid search combines dense vectors with sparse keyword matching
    - GraphQL queries provide flexible result shaping
    - Modular AI integrations simplify embedding management

Architecture Notes:
    Weaviate's hybrid search is particularly valuable for agentic RAG when
    the agent encounters queries with specific terminology that semantic
    search might miss.
"""
...
class WeaviateAgenticRAGPipeline(AgenticRAGPipeline):
    """Weaviate agentic RAG pipeline (LangChain).

    Combines agentic routing with vector search, document compression,
    answer reflection, and RAG generation using Weaviate backend.

    Weaviate's AI-native design provides integrated capabilities that
    simplify agentic RAG deployment and enhance retrieval quality.

    Attributes:
        config: Loaded and validated configuration dictionary.
        embedder: Embedding model for query vectorization.
        db: WeaviateVectorDB instance for AI-native document retrieval.
        collection_name: Target Weaviate collection (class) for search.
    ...
    def __init__(self, config_or_path: dict[str, Any] | str) -> None:
        ...
        Sets up the Weaviate connection, embedding model, LLM, router, and
        document compressor.

        Args:
            config_or_path: Configuration dictionary or path to YAML file.
                Must contain weaviate, embedding, rag, and agentic sections.
        ...
        logger.info(
            "Initialized Weaviate agentic RAG pipeline (LangChain) "
    ...
    def run(
        ...
        Runs the iterative agent loop with Weaviate as the retrieval backend.
        Weaviate's schema properties enable rich filtering that the agent
        can leverage during reasoning.
