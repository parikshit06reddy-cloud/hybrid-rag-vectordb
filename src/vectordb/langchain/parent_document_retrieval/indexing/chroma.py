"""Chroma parent document retrieval indexing pipeline (LangChain).

This module implements the indexing pipeline for parent document retrieval
using Chroma as the vector database. It chunks documents while maintaining
...
Pipeline Flow:
    ...
    5. Create Chroma collection (optionally recreating)
...
Example:
    >>> pipeline = ChromaParentDocumentRetrievalIndexingPipeline("config.yaml")
    >>> result = pipeline.run()
...
class ChromaParentDocumentRetrievalIndexingPipeline:
    ...
    def __init__(self, config_or_path: dict[str, Any] | str) -> None:
        ...
        Loads configuration, initializes Chroma connection, sets up text
        splitter, and creates the parent document store.

        Args:
            config_or_path: Config dict or path to YAML configuration file.
                Must contain chroma, embedder, and chunking sections.
        ...
        logger.info(
            "Initialized Chroma parent document retrieval indexing pipeline (LangChain)"
        )
    ...
    def run(self) -> dict[str, Any]:
        ...
        # Create or recreate Chroma collection
        ...
        for chunk, embedding in zip(all_chunks, embeddings):
            ...
            # Build document with parent reference and embedding
            upsert_data.append(
                {
                    "id": chunk_id,
                    "text": chunk["text"],
        ...
        logger.info("Indexed %d chunks to Chroma", num_indexed)
