"""Qdrant parent document retrieval search pipeline (LangChain).

This module implements the search pipeline for parent document retrieval
using Qdrant as the vector database. It retrieves chunks by similarity
...
Search Pipeline Flow:
    ...
    3. For each search query:
        ...
        b. Search Qdrant for similar chunks (retrieves top_k * 2)
...
Example:
    >>> searcher = QdrantParentDocumentRetrievalSearchPipeline("config.yaml")
...
class QdrantParentDocumentRetrievalSearchPipeline:
    """Qdrant search pipeline for parent document retrieval (LangChain).

    Retrieves chunk embeddings from Qdrant, maps chunks to parent documents
    ...
    def __init__(self, config_or_path: dict[str, Any] | str) -> None:
        ...
        Loads configuration, initializes Qdrant connection, sets up embedder,
        and loads the ParentDocumentStore from disk.

        Args:
            config_or_path: Config dict or path to YAML configuration file.
                Must contain qdrant, embedder, and parent_store sections.
        ...
        logger.info(
            "Initialized Qdrant parent document retrieval search pipeline (LangChain)"
        )
    ...
    def search(
        ...
        Search Strategy:
            1. Embed query text
            2. Search Qdrant for top_k * 2 chunks (extra for deduplication)
        ...
        logger.info("Retrieved %d chunks from Qdrant", len(chunk_documents))
