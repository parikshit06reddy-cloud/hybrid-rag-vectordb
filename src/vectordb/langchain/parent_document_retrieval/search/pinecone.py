"""Pinecone parent document retrieval search pipeline (LangChain).

This module implements the search pipeline for parent document retrieval
using Pinecone as the vector database. It retrieves chunks by similarity
...
Search Pipeline Flow:
    ...
    3. For each search query:
        ...
        b. Search Pinecone for similar chunks (retrieves top_k * 2)
...
class PineconeParentDocumentRetrievalSearchPipeline:
    """Pinecone search pipeline for parent document retrieval (LangChain).

    Retrieves chunk embeddings from Pinecone, maps chunks to parent documents
    ...
    Attributes:
        ...
        db: PineconeVectorDB instance for vector search
        index_name: Name of Pinecone index
    ...
    Example:
        >>> searcher = PineconeParentDocumentRetrievalSearchPipeline(
        ...     {
        ...         "pinecone": {
        ...             "api_key": "your-key",
        ...             "index_name": "my-index",
    ...
    def __init__(self, config_or_path: dict[str, Any] | str) -> None:
        ...
        Loads configuration, initializes Pinecone connection, sets up embedder,
        and loads the ParentDocumentStore from disk.

        Args:
            config_or_path: Config dict or path to YAML configuration file.
                Must contain pinecone, embedder, and parent_store sections.
        ...
        logger.info(
            "Initialized Pinecone parent document retrieval search pipeline (LangChain)"
        )
    ...
    def search(
        ...
        Search Strategy:
            1. Embed query text
            2. Search Pinecone for top_k * 2 chunks (extra for deduplication)
        ...
        logger.info("Retrieved %d chunks from Pinecone", len(chunk_documents))
