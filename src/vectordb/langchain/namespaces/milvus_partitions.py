...
def main():
    ...
    # Milvus parameters
    parser.add_argument(
        "--milvus_uri", default="http://localhost:19530", help="URI for Milvus server."
    )
    parser.add_argument(
        "--milvus_token", default="root:Milvus", help="Token for Milvus authentication."
    )
    parser.add_argument(
        "--collection_name", required=True, help="Milvus collection name."
    )
    parser.add_argument(
        "--partition_name", required=True, help="Partition name in Milvus collection."
    )
    parser.add_argument(
        "--query", type=str, required=True, help="Query string for Milvus search."
    )
    ...
    if args.partition_name not in [
        partition.name for partition in collection.partitions
    ]:
