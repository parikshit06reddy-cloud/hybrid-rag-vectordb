"""Tests for Milvus namespace search pipeline (LangChain)."""
...
class TestMilvusNamespaceSearchPipeline:
    """Unit tests for Milvus namespace search pipeline."""
    ...
    def test_init_with_valid_namespace(
        self, mock_llm, mock_embedder, mock_db_cls, milvus_namespace_config: dict
    ):
        ...
        pipeline = MilvusNamespaceSearchPipeline(milvus_namespace_config, "ns_123")

        assert pipeline.namespace == "ns_123"
        assert pipeline.config == milvus_namespace_config
    ...
    def test_init_with_empty_namespace_raises_error(
        ...
        with pytest.raises(ValueError, match="namespace cannot be empty"):
            MilvusNamespaceSearchPipeline(milvus_namespace_config, "")
    ...
    def test_search_returns_documents(
        ...
        mock_embedder,
        mock_db_cls,
        milvus_namespace_config: dict,
    ):
        """Test search returns documents from namespace-specific partition."""
    ...
    @patch("vectordb.langchain.utils.RAGHelper.create_llm")
    @patch("vectordb.langchain.utils.EmbedderHelper.embed_query")
    def test_search_uses_partition_name(
        self,
        mock_embed_query,
        mock_llm,
        mock_embedder,
        ...
        """Test search queries namespace-specific partition."""
        mock_embedder_instance = MagicMock()
        mock_embedder.return_value = mock_embedder_instance
        mock_llm.return_value = None
        mock_embed_query.return_value = [0.1] * 384
        ...
        pipeline = MilvusNamespaceSearchPipeline(milvus_namespace_config, "ns_abc")
        pipeline.search("test query")

        call_kwargs = mock_db_instance.query.call_args.kwargs
        assert call_kwargs["partition_name"] == "ns_abc"
