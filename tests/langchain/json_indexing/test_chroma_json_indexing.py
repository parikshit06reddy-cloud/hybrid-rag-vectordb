...
class TestChromaJSONIndexing:
    ...
    def test_indexing_run_no_documents(
        ...
        mock_db_inst.upsert.assert_not_called()
    ...
    def test_indexing_with_recreate(
        ...
        """Test indexing with collection recreation option.

        Verifies that when recreate=True:
        - Existing collection is deleted before creating new one
        - Documents are indexed to fresh collection

        Args:
            mock_get_docs: Mock returning sample_documents fixture.
            mock_embed_docs: Mock returning (docs, embeddings) tuple.
            mock_embedder_helper: Mock for embedder factory.
            mock_db: Mock for ChromaVectorDB, tracks delete/create calls.
            sample_documents: Fixture with sample documents.
        """
        mock_dataset = MagicMock()
        mock_dataset.to_langchain.return_value = sample_documents
