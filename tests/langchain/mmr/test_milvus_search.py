"""Unit tests for Milvus MMR search pipeline (LangChain).

Tests validate the MilvusMMRSearchPipeline's behavior for Maximal Marginal
Relevance (MMR) search with Milvus vector database. MMR balances relevance
against diversity to reduce redundant results in retrieval.

Milvus-specific aspects tested include:
- Host/port connection configuration for gRPC/REST
...
class TestMilvusMMRSearchPipeline:
    """Test suite for Milvus MMR search pipeline.

    Validates pipeline initialization and search execution:
    - Config parsing with Milvus-specific parameters
    - Collection name extraction for document organization
    ...
    @patch("vectordb.langchain.mmr.search.milvus.MilvusVectorDB")
    @patch("vectordb.langchain.mmr.search.milvus.EmbedderHelper.create_embedder")
    def test_init_with_valid_config(
        ...
        Validates that:
        - Pipeline extracts collection_name from milvus config
        - Host and port parameters are preserved for connection
        - Embedder is initialized via EmbedderHelper
        - MilvusVectorDB is instantiated with config

        Args:
            mock_embedder_helper: Mock for EmbedderHelper.create_embedder.
            mock_db_cls: Mock for MilvusVectorDB class.
            milvus_config: Fixture with Milvus configuration dict.
        ...
        pipeline = MilvusMMRSearchPipeline(milvus_config)
    ...
    def test_init_with_config_path(
        ...
        - All Milvus parameters (host, port, collection_name) extracted
...
mmr:
  ...
  k: 5
        ...
        pipeline = MilvusMMRSearchPipeline(str(config_file))
        assert pipeline is not None
    ...
    class TestSearch:
        ...
        @patch("vectordb.langchain.mmr.search.milvus.MilvusVectorDB")
        @patch("vectordb.langchain.mmr.search.milvus.EmbedderHelper.create_embedder")
        @patch("vectordb.langchain.mmr.search.milvus.EmbedderHelper.embed_query")
        @patch("vectordb.langchain.mmr.search.milvus.MMRHelper.mmr_rerank_simple")
        def test_search_returns_documents(
            ...
            2. MilvusVectorDB.query retrieves candidate documents
            ...
            Args:
                ...
                mock_embedder_helper: Mock for EmbedderHelper.create_embedder.
                mock_db_cls: Mock for MilvusVectorDB class.
                milvus_config: Fixture with Milvus configuration.
        ...
        def test_search_with_filters(
            ...
            Validates that:
            - Filters dict is passed to MilvusVectorDB.query
            - Milvus filter format applied to metadata fields
        ...
        def test_search_with_top_k(
            ...
            Validates that:
            - top_k is passed to MilvusVectorDB.query
