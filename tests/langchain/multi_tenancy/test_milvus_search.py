"""Tests for Milvus multi-tenancy search pipeline (LangChain)."""
...
class TestMilvusMultiTenancySearchPipeline:
    """Unit tests for Milvus multi-tenancy search pipeline."""
    ...
    def test_init_with_valid_tenant_id(
        self, mock_llm, mock_embedder, mock_db_cls, milvus_multi_tenant_config: dict
    ):
        ...
        assert pipeline.config == milvus_multi_tenant_config
    ...
    def test_search_returns_documents_from_tenant_partition(
        self,
        mock_embed_query,
        mock_llm,
        mock_embedder,
        mock_db_cls,
        milvus_multi_tenant_config: dict,
    ):
        """Test search returns documents from tenant-specific partition."""
    ...
    @patch("vectordb.langchain.utils.RAGHelper.create_llm")
    @patch("vectordb.langchain.utils.EmbedderHelper.embed_query")
    def test_search_uses_tenant_partition(
        self,
        mock_embed_query,
        mock_llm,
        mock_embedder,
        ...
        """Test search queries tenant-specific partition."""
        mock_embedder_instance = MagicMock()
        mock_embedder.return_value = mock_embedder_instance
        mock_llm.return_value = None
        mock_embed_query.return_value = [0.1] * 384
        ...
        pipeline = MilvusMultiTenancySearchPipeline(
            milvus_multi_tenant_config, "tenant_abc"
        )
        pipeline.search("test query")

        call_kwargs = mock_db_instance.query.call_args.kwargs
        assert call_kwargs["partition_name"] == "tenant_abc"
    ...
    def test_search_different_tenants_get_isolated_results(
        ...
        pipeline = MilvusMultiTenancySearchPipeline(
            milvus_multi_tenant_config, "tenant_A"
        )
