"""Tests for Pinecone multi-tenancy indexing pipeline (LangChain)."""
...
class TestPineconeMultiTenancyIndexingPipeline:
    """Unit tests for Pinecone multi-tenancy indexing pipeline."""
    ...
    def test_init_with_valid_tenant_id(
        self, mock_embedder, mock_db_cls, pinecone_multi_tenant_config: dict
    ):
    ...
    @patch("vectordb.dataloaders.DataloaderCatalog.create")
    @patch("vectordb.langchain.utils.EmbedderHelper.embed_documents")
    def test_indexing_uses_tenant_namespace(
        self,
        mock_embed_documents,
        mock_get_documents,
        mock_embedder,
        ...
        """Test indexing uses tenant-specific namespace."""
        mock_embedder.return_value = MagicMock()
    ...
    def test_indexing_with_limit_respects_limit(
        ...
        # Add limit to config
        pinecone_multi_tenant_config["dataloader"]["limit"] = 2
