"""Tests for Qdrant multi-tenancy search pipeline (LangChain)."""
...
class TestQdrantMultiTenancySearchPipeline:
    """Unit tests for Qdrant multi-tenancy search pipeline."""
    ...
    def test_init_with_valid_tenant_id(
        self, mock_llm, mock_embedder, mock_db_cls, qdrant_multi_tenant_config: dict
    ):
    ...
    def test_search_different_tenants_get_isolated_results(
        ...
        pipeline = QdrantMultiTenancySearchPipeline(
            qdrant_multi_tenant_config, "tenant_A"
        )
