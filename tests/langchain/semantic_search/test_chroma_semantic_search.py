"""Tests for Chroma semantic search pipelines using LangChain.

This module tests the semantic search pipeline implementation for Chroma vector
database. It validates both the indexing pipeline (document loading, embedding,
storage) and the search pipeline (query embedding, vector retrieval, RAG generation).

Indexing Pipeline:
    - Loads documents from configured dataloaders (ARC, TriviaQA, PopQA, etc.)
    - Embeds documents using specified embedding models
    - Stores vectors and metadata in Chroma collections
    - Supports local persistent storage via file paths

Search Pipeline:
    - Embeds user queries using the same embedding model
    - Performs vector similarity search against Chroma collections
    - Returns ranked documents by relevance score
    - Optional RAG mode: Generates answers using retrieved context

Database Configuration:
    - path: Local directory for Chroma persistent storage
...
class TestChromaSemanticIndexing:
    """Unit tests for Chroma semantic indexing pipeline.

    Tests the indexing pipeline which orchestrates document loading, embedding,
    and vector storage operations for Chroma vector database.

    Pipeline Flow:
        1. Load documents from configured dataloader with limit
        2. Embed documents using EmbedderHelper with specified model
        3. Initialize ChromaVectorDB with path and collection name
        4. Upsert embeddings and metadata into Chroma collection
        5. Return indexing statistics
    """

    @patch("vectordb.langchain.semantic_search.indexing.chroma.ChromaVectorDB")
    @patch(
        "vectordb.langchain.semantic_search.indexing.chroma.EmbedderHelper.create_embedder"
    )
    @patch(
        "vectordb.langchain.semantic_search.indexing.chroma.DataloaderCatalog.create"
    )
    def test_indexing_initialization(
        self, mock_get_docs, mock_embedder_helper, mock_db
    ):
        """Test pipeline initialization with Chroma-specific configuration.
        ...
        Verifies that the indexing pipeline correctly initializes with:
            ...
            - Chroma connection parameters (local path, collection name)
    ...
    @patch(
        "vectordb.langchain.semantic_search.indexing.chroma.EmbedderHelper.embed_documents"
    )
    @patch(
        "vectordb.langchain.semantic_search.indexing.chroma.DataloaderCatalog.create"
    )
    def test_indexing_run_with_documents(
        ...
        Validates the complete indexing flow:
            ...
            3. ChromaVectorDB is initialized with config parameters
            4. Embeddings are upserted into Chroma collection
        ...
        Args:
            ...
            mock_db: Mock for ChromaVectorDB with upsert tracking
...
class TestChromaSemanticSearch:
    """Unit tests for Chroma semantic search pipeline.

    Tests the search pipeline which orchestrates query embedding, vector search,
    and optional RAG-based answer generation for Chroma vector database.

    Pipeline Flow (RAG disabled):
        1. Embed user query using configured embedding model
        2. Query ChromaVectorDB with embedded query vector
    ...
    @patch("vectordb.langchain.semantic_search.search.chroma.ChromaVectorDB")
    @patch(
        "vectordb.langchain.semantic_search.search.chroma.EmbedderHelper.create_embedder"
    )
    @patch("vectordb.langchain.semantic_search.search.chroma.RAGHelper.create_llm")
    def test_search_initialization(self, mock_llm, mock_embedder_helper, mock_db):
        ...
        Verifies that when RAG is disabled:
            ...
            - ChromaVectorDB is initialized with config parameters
    ...
    @patch(
        "vectordb.langchain.semantic_search.search.chroma.EmbedderHelper.embed_query"
    )
    @patch("vectordb.langchain.semantic_search.search.chroma.RAGHelper.create_llm")
    def test_search_execution(
        ...
        Validates the search flow without answer generation:
            ...
            2. ChromaVectorDB.query is called with embedded query
    ...
    @patch("vectordb.langchain.semantic_search.search.chroma.RAGHelper.create_llm")
    @patch("vectordb.langchain.semantic_search.search.chroma.RAGHelper.generate")
    def test_search_with_rag(
        ...
        Validates the search flow with answer generation:
            1. Query is embedded into 384-dimensional vector
            2. ChromaVectorDB.query retrieves relevant documents
