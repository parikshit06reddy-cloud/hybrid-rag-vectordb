"""Tests for Milvus semantic search pipelines using LangChain.

This module tests the semantic search pipeline implementation for Milvus vector
database.
...
Indexing Pipeline:
    ...
    - Stores vectors and metadata in Milvus collections
    - Supports Milvus server connections via URI

Search Pipeline:
    - Embeds user queries using the same embedding model
    - Performs vector similarity search against Milvus collections
...
class TestMilvusSemanticIndexing:
    """Unit tests for Milvus semantic indexing pipeline.

    Tests the indexing pipeline which orchestrates document loading, embedding,
    and vector storage operations for Milvus vector database.
    ...
    @patch(
        "vectordb.langchain.semantic_search.indexing.milvus.EmbedderHelper.create_embedder"
    )
    @patch(
        "vectordb.langchain.semantic_search.indexing.milvus.DataloaderCatalog.create"
    )
    def test_indexing_initialization(
        self, mock_get_docs, mock_embedder_helper, mock_db
    ):
        """Test pipeline initialization with Milvus-specific configuration.
        ...
        Verifies that the indexing pipeline correctly initializes with:
            ...
            - Milvus connection parameters (server URI, collection name)
    ...
    @patch(
        "vectordb.langchain.semantic_search.indexing.milvus.EmbedderHelper.embed_documents"
    )
    @patch(
        "vectordb.langchain.semantic_search.indexing.milvus.DataloaderCatalog.create"
    )
...
class TestMilvusSemanticSearch:
    """Unit tests for Milvus semantic search pipeline.

    Tests the search pipeline which orchestrates query embedding, vector search,
    and optional RAG-based answer generation for Milvus vector database.
    ...
    @patch("vectordb.langchain.semantic_search.search.milvus.MilvusVectorDB")
    @patch(
        "vectordb.langchain.semantic_search.search.milvus.EmbedderHelper.create_embedder"
    )
    @patch("vectordb.langchain.semantic_search.search.milvus.RAGHelper.create_llm")
    def test_search_initialization(self, mock_llm, mock_embedder_helper, mock_db):
        ...
        Verifies that when RAG is disabled:
            ...
            - MilvusVectorDB is initialized with config parameters
    ...
    @patch(
        "vectordb.langchain.semantic_search.search.milvus.EmbedderHelper.embed_query"
    )
    @patch("vectordb.langchain.semantic_search.search.milvus.RAGHelper.create_llm")
    def test_search_execution(
        ...
        Args:
            ...
            mock_embedder_helper: Mock for embedder initialization
            mock_db: Mock for MilvusVectorDB with query results
            sample_documents: Pytest fixture providing test documents
        """
    ...
    @patch("vectordb.langchain.semantic_search.search.milvus.RAGHelper.create_llm")
    @patch("vectordb.langchain.semantic_search.search.milvus.RAGHelper.generate")
