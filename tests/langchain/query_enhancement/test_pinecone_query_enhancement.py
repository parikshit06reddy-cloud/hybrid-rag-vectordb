"""Tests for Pinecone query enhancement pipelines (LangChain)."""
...
class TestPineconeQueryEnhancementIndexing:
    """Unit tests for Pinecone query enhancement indexing pipeline."""

    @patch("vectordb.langchain.query_enhancement.indexing.pinecone.PineconeVectorDB")
    @patch(
        "vectordb.langchain.query_enhancement.indexing.pinecone.EmbedderHelper.create_embedder"
    )
    @patch(
        "vectordb.langchain.query_enhancement.indexing.pinecone.DataloaderCatalog.create"
    )
    def test_indexing_initialization(
        ...
        mock_db,
        pinecone_query_enhancement_config,
    ):
        """Test pipeline initialization."""
        pipeline = PineconeQueryEnhancementIndexingPipeline(
            pinecone_query_enhancement_config
        )
        assert pipeline.config == pinecone_query_enhancement_config
        assert pipeline.index_name == "test-query-enhancement"
    ...
    @patch(
        "vectordb.langchain.query_enhancement.indexing.pinecone.EmbedderHelper.embed_documents"
    )
    @patch(
        "vectordb.langchain.query_enhancement.indexing.pinecone.DataloaderCatalog.create"
    )
    def test_indexing_run_with_documents(
        ...
        pipeline = PineconeQueryEnhancementIndexingPipeline(config)
        result = pipeline.run()
...
class TestPineconeQueryEnhancementSearch:
    """Unit tests for Pinecone query enhancement search pipeline."""

    @patch("langchain_groq.ChatGroq")
    @patch("vectordb.langchain.query_enhancement.search.pinecone.PineconeVectorDB")
    @patch(
    ...
    def test_search_initialization(
        ...
        pipeline = PineconeQueryEnhancementSearchPipeline(config)
