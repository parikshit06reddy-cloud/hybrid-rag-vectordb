"""Tests for Pinecone parent document retrieval pipelines (LangChain)."""
...
class TestPineconeParentDocumentRetrievalIndexing:
    """Unit tests for Pinecone parent document retrieval indexing pipeline."""

    @patch(
        "vectordb.langchain.parent_document_retrieval.indexing.pinecone.PineconeVectorDB"
    )
    @patch(
        "vectordb.langchain.parent_document_retrieval.indexing.pinecone.EmbedderHelper.create_embedder"
    )
    @patch(
        "vectordb.langchain.parent_document_retrieval.indexing.pinecone.DataloaderCatalog.create"
    )
    def test_indexing_initialization(
        ...
        pipeline = PineconeParentDocumentRetrievalIndexingPipeline(config)
        assert pipeline.config == config
        assert pipeline.index_name == "test-parent-document-retrieval"
    ...
    @patch(
        "vectordb.langchain.parent_document_retrieval.indexing.pinecone.EmbedderHelper.embed_documents"
    )
    @patch(
        "vectordb.langchain.parent_document_retrieval.indexing.pinecone.DataloaderCatalog.create"
    )
...
class TestPineconeParentDocumentRetrievalSearch:
    """Unit tests for Pinecone parent document retrieval search pipeline."""

    @patch(
        "vectordb.langchain.parent_document_retrieval.search.pinecone.PineconeVectorDB"
    )
    @patch(
        "vectordb.langchain.parent_document_retrieval.search.pinecone.EmbedderHelper.create_embedder"
    )
    @patch(
        "vectordb.langchain.parent_document_retrieval.search.pinecone.RAGHelper.create_llm"
    )
    def test_search_initialization(
        ...
        pipeline = PineconeParentDocumentRetrievalSearchPipeline(config)
        assert pipeline.config == config
        assert pipeline.index_name == "test-parent-document-retrieval"
        assert pipeline.namespace == "test-namespace"
    ...
    @patch(
        "vectordb.langchain.parent_document_retrieval.search.pinecone.EmbedderHelper.embed_query"
    )
    @patch(
        "vectordb.langchain.parent_document_retrieval.search.pinecone.RAGHelper.create_llm"
    )
    ...
    @patch(
        "vectordb.langchain.parent_document_retrieval.search.pinecone.RAGHelper.generate"
    )
