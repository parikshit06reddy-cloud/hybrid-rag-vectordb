"""Tests for Weaviate cost-optimized RAG indexing pipeline (LangChain)."""
...
class TestWeaviateCostOptimizedRAGIndexingPipeline:
    """Tests for WeaviateCostOptimizedRAGIndexingPipeline."""
    ...
    def test_init_with_valid_config(
        ...
        pipeline = WeaviateCostOptimizedRAGIndexingPipeline(weaviate_config)
...
chunking:
  ...
  chunk_overlap: 200
        ...
        pipeline = WeaviateCostOptimizedRAGIndexingPipeline(str(config_file))
        assert pipeline is not None
    ...
    class TestRun:
        ...
        @patch(
            "vectordb.langchain.cost_optimized_rag.indexing.weaviate.WeaviateVectorDB"
        )
        @patch(
            "vectordb.langchain.cost_optimized_rag.indexing.weaviate.EmbedderHelper.create_embedder"
        )
