"""Tests for Qdrant reranking pipeline (indexing and search)."""
...
class TestQdrantReranking:
    """Unit tests for Qdrant reranking pipeline (indexing and search)."""

    # Indexing tests
    @patch(
        "vectordb.haystack.reranking.indexing.qdrant.EmbedderFactory.get_embedding_dimension"
    )
    @patch("vectordb.haystack.reranking.indexing.qdrant.QdrantVectorDB")
    @patch(
        "vectordb.haystack.reranking.indexing.qdrant.EmbedderFactory.create_document_embedder"
    )
    @patch("vectordb.haystack.reranking.indexing.qdrant.DataloaderCatalog.create")
    def test_indexing_init_loads_config(
        ...
        pipeline = QdrantRerankingIndexingPipeline(qdrant_config)
    ...
    def test_indexing_init_invalid_config(self) -> None:
        ...
        with pytest.raises(ValueError):
            QdrantRerankingIndexingPipeline(invalid_config)

    # Search tests
    @patch("vectordb.haystack.reranking.search.qdrant.QdrantVectorDB")
    @patch(
        "vectordb.haystack.reranking.search.qdrant.EmbedderFactory.create_text_embedder"
    )
    @patch("vectordb.haystack.reranking.search.qdrant.RerankerFactory.create")
    def test_search_init_loads_config(
        ...
        pipeline = QdrantRerankingSearchPipeline(qdrant_config)
        assert pipeline.config == qdrant_config
    ...
    def test_search_init_invalid_config(self) -> None:
        ...
        invalid_config = {"embeddings": {"model": "test"}}
        with pytest.raises(ValueError):
            QdrantRerankingSearchPipeline(invalid_config)
