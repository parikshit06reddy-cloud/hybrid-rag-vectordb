"""Tests for Weaviate reranking pipeline (indexing and search)."""
...
class TestWeaviateReranking:
    """Unit tests for Weaviate reranking pipeline (indexing and search)."""

    # Indexing tests
    @patch(
        "vectordb.haystack.reranking.indexing.weaviate.EmbedderFactory.get_embedding_dimension"
    )
    @patch("vectordb.haystack.reranking.indexing.weaviate.WeaviateVectorDB")
    @patch(
        "vectordb.haystack.reranking.indexing.weaviate.EmbedderFactory.create_document_embedder"
    )
    @patch("vectordb.haystack.reranking.indexing.weaviate.DataloaderCatalog.create")
    def test_indexing_init_loads_config(
        ...
        pipeline = WeaviateRerankingIndexingPipeline(weaviate_config)
    ...
    def test_indexing_init_invalid_config(self) -> None:
        ...
        with pytest.raises(ValueError):
            WeaviateRerankingIndexingPipeline(invalid_config)

    # Search tests
    @patch("vectordb.haystack.reranking.search.weaviate.WeaviateVectorDB")
    @patch(
        "vectordb.haystack.reranking.search.weaviate.EmbedderFactory.create_text_embedder"
    )
    @patch("vectordb.haystack.reranking.search.weaviate.RerankerFactory.create")
    def test_search_init_loads_config(
        ...
        pipeline = WeaviateRerankingSearchPipeline(weaviate_config)
        assert pipeline.config == weaviate_config
    ...
    def test_search_init_invalid_config(self) -> None:
        ...
        invalid_config = {"embeddings": {"model": "test"}}
        with pytest.raises(ValueError):
            WeaviateRerankingSearchPipeline(invalid_config)
    ...
    def test_search_with_custom_top_k(
        ...
        assert call_args.kwargs["limit"] == 9  # 3 * 3
