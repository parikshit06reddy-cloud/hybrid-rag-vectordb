"""Tests for Milvus indexing pipeline in diversity filtering."""
...
from vectordb.haystack.diversity_filtering.pipelines.milvus_indexing import (
    load_documents,
    run_indexing,
)
...
class TestLoadDocuments:
    ...
    @patch(
        "vectordb.haystack.diversity_filtering.pipelines.milvus_indexing.DataloaderCatalog"
    )
...
class TestRunIndexing:
    ...
    def mock_config(self) -> MagicMock:
        ...
        config.vectordb.milvus.host = "localhost"
        config.vectordb.milvus.port = 19530
        config.vectordb.milvus.db_name = "default"
        return config

    @patch(
        "vectordb.haystack.diversity_filtering.pipelines.milvus_indexing.MilvusVectorDB"
    )
    @patch(
        "vectordb.haystack.diversity_filtering.pipelines.milvus_indexing.SentenceTransformersDocumentEmbedder"
    )
    @patch(
        "vectordb.haystack.diversity_filtering.pipelines.milvus_indexing.ConfigLoader"
    )
    @patch(
        "vectordb.haystack.diversity_filtering.pipelines.milvus_indexing.load_documents"
    )
    ...
    def test_run_indexing_db_initialization(
        ...
        """Test MilvusVectorDB initialization parameters."""
        config_file = tmp_path / "config.yaml"
        config_file.write_text("dummy: config")
    ...
    def test_run_indexing_custom_db_name(
        self,
        mock_load_documents: MagicMock,
        mock_config_loader: MagicMock,
        mock_embedder_class: MagicMock,
        ...
        mock_config.vectordb.milvus.db_name = "custom_db"
