"""Tests for Pinecone search pipeline in diversity filtering."""
...
class TestRunSearch:
    ...
    def mock_config(self) -> MagicMock:
        ...
        config.vectordb.pinecone.api_key = "test-api-key"
        config.vectordb.pinecone.index_name = "pinecone-test-index"
        return config
    ...
    @patch(
        "vectordb.haystack.diversity_filtering.pipelines.pinecone_search.SentenceTransformersDiversityRanker"
    )
    @patch(
        "vectordb.haystack.diversity_filtering.pipelines.pinecone_search.PineconeVectorDB"
    )
    @patch(
        "vectordb.haystack.diversity_filtering.pipelines.pinecone_search.SentenceTransformersTextEmbedder"
    )
    @patch(
        "vectordb.haystack.diversity_filtering.pipelines.pinecone_search.ConfigLoader"
    )
    ...
    @patch(
        "vectordb.haystack.diversity_filtering.pipelines.pinecone_search.OpenAIGenerator"
    )
    @patch(
        "vectordb.haystack.diversity_filtering.pipelines.pinecone_search.PromptBuilder"
    )
    @patch(
        "vectordb.haystack.diversity_filtering.pipelines.pinecone_search.format_documents"
    )
    @patch(
        "vectordb.haystack.diversity_filtering.pipelines.pinecone_search.get_prompt_template"
    )
    @patch(
        "vectordb.haystack.diversity_filtering.pipelines.pinecone_search.SentenceTransformersDiversityRanker"
    )
    ...
    def test_run_search_db_initialization(
        ...
        """Test PineconeVectorDB initialization in search."""
        config_file = tmp_path / "config.yaml"
        config_file.write_text("dummy: config")
        ...
        mock_db_class.assert_called_once_with(
            ...
            index_name="pinecone-test-index",
            embedding_dim=384,
        )
