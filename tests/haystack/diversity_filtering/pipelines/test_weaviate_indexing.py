"""Tests for Weaviate indexing pipeline in diversity filtering."""
...
from vectordb.haystack.diversity_filtering.pipelines.weaviate_indexing import (
    load_documents,
    run_indexing,
)
...
class TestLoadDocuments:
    ...
    @patch(
        "vectordb.haystack.diversity_filtering.pipelines.weaviate_indexing.DataloaderCatalog"
    )
    ...
    def test_load_documents_no_text_or_content(self, mock_catalog: MagicMock) -> None:
        ...
        mock_config = MagicMock()
        mock_config.dataset.name = "popqa"
        mock_config.dataset.split = "test"
...
class TestRunIndexing:
    ...
    def mock_config(self) -> MagicMock:
        ...
        config.vectordb.weaviate.url = "http://localhost:8080"
        config.vectordb.weaviate.api_key = None
        return config

    @patch(
        "vectordb.haystack.diversity_filtering.pipelines.weaviate_indexing.WeaviateVectorDB"
    )
    @patch(
        "vectordb.haystack.diversity_filtering.pipelines.weaviate_indexing.SentenceTransformersDocumentEmbedder"
    )
    @patch(
        "vectordb.haystack.diversity_filtering.pipelines.weaviate_indexing.ConfigLoader"
    )
    @patch(
        "vectordb.haystack.diversity_filtering.pipelines.weaviate_indexing.load_documents"
    )
    ...
    def test_run_indexing_db_initialization(
        self,
        mock_load_documents: MagicMock,
        mock_config_loader: MagicMock,
        mock_embedder_class: MagicMock,
        ...
        """Test WeaviateVectorDB initialization parameters."""
        config_file = tmp_path / "config.yaml"
        config_file.write_text("dummy: config")
    ...
    def test_run_indexing_with_api_key(
        self,
        mock_load_documents: MagicMock,
        mock_config_loader: MagicMock,
        mock_embedder_class: MagicMock,
        ...
        """Test indexing with API key."""
        mock_config.vectordb.weaviate.api_key = "secret-api-key"
