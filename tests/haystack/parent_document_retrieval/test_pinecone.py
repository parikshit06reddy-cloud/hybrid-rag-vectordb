"""Tests for Pinecone parent document retrieval pipeline (indexing and search)."""
...
class TestPineconeParentDocIndexing:
    """Unit tests for Pinecone parent document indexing pipeline."""

    @patch(
        "vectordb.haystack.parent_document_retrieval.indexing.pinecone.load_parent_doc_config"
    )
    @patch("vectordb.haystack.parent_document_retrieval.indexing.pinecone.setup_logger")
    @patch(
        "vectordb.haystack.parent_document_retrieval.indexing.pinecone.SentenceTransformersDocumentEmbedder"
    )
    @patch(
        "vectordb.haystack.parent_document_retrieval.indexing.pinecone.HierarchicalDocumentSplitter"
    )
    @patch(
        "vectordb.haystack.parent_document_retrieval.indexing.pinecone.PineconeVectorDB"
    )
    def test_indexing_init_loads_config(
        ...
        from vectordb.haystack.parent_document_retrieval.indexing.pinecone import (
    ...
    @patch(
        "vectordb.haystack.parent_document_retrieval.indexing.pinecone.DataloaderCatalog"
    )
...
class TestPineconeParentDocSearch:
    """Unit tests for Pinecone parent document search pipeline."""

    @patch(
        "vectordb.haystack.parent_document_retrieval.search.pinecone.load_parent_doc_config"
    )
    @patch("vectordb.haystack.parent_document_retrieval.search.pinecone.setup_logger")
    @patch(
        "vectordb.haystack.parent_document_retrieval.search.pinecone.SentenceTransformersTextEmbedder"
    )
    @patch(
        "vectordb.haystack.parent_document_retrieval.search.pinecone.PineconeVectorDB"
    )
    @patch(
        "vectordb.haystack.parent_document_retrieval.search.pinecone.AutoMergingRetriever"
    )
    def test_search_init_loads_config(
        ...
        from vectordb.haystack.parent_document_retrieval.search.pinecone import (
            PineconeParentDocSearchPipeline,
        )

        mock_load_config.return_value = pinecone_config

        pipeline = PineconeParentDocSearchPipeline("config.yaml", sample_parent_store)
    ...
    def test_search_initializes_auto_merger_with_threshold(
        ...
        PineconeParentDocSearchPipeline("config.yaml", sample_parent_store)
