"""Tests for Qdrant parent document retrieval pipeline (indexing and search)."""
...
class TestQdrantParentDocIndexing:
    """Unit tests for Qdrant parent document indexing pipeline."""

    @patch(
        "vectordb.haystack.parent_document_retrieval.indexing.qdrant.load_parent_doc_config"
    )
    @patch("vectordb.haystack.parent_document_retrieval.indexing.qdrant.setup_logger")
    @patch(
        "vectordb.haystack.parent_document_retrieval.indexing.qdrant.SentenceTransformersDocumentEmbedder"
    )
    @patch(
        "vectordb.haystack.parent_document_retrieval.indexing.qdrant.HierarchicalDocumentSplitter"
    )
    @patch("vectordb.haystack.parent_document_retrieval.indexing.qdrant.QdrantVectorDB")
    def test_indexing_init_loads_config(
        ...
        from vectordb.haystack.parent_document_retrieval.indexing.qdrant import (
            QdrantParentDocIndexingPipeline,
        )

        mock_load_config.return_value = qdrant_config

        pipeline = QdrantParentDocIndexingPipeline("config.yaml")
    ...
    @patch("vectordb.haystack.parent_document_retrieval.indexing.qdrant.QdrantVectorDB")
    @patch(
        "vectordb.haystack.parent_document_retrieval.indexing.qdrant.DataloaderCatalog"
    )
    def test_indexing_run_creates_hierarchy(
        ...
        mock_db = MagicMock()
        mock_db.index_documents.return_value = len(sample_leaf_documents)
    ...
    def test_indexing_creates_collection(
        ...
        QdrantParentDocIndexingPipeline("config.yaml")

        mock_db.create_collection.assert_called_once()
        call_kwargs = mock_db.create_collection.call_args[0]
        assert call_kwargs[0] == 1024  # dimension is first positional arg
...
class TestQdrantParentDocSearch:
    """Unit tests for Qdrant parent document search pipeline."""

    @patch(
        "vectordb.haystack.parent_document_retrieval.search.qdrant.load_parent_doc_config"
    )
    @patch("vectordb.haystack.parent_document_retrieval.search.qdrant.setup_logger")
    @patch(
        "vectordb.haystack.parent_document_retrieval.search.qdrant.SentenceTransformersTextEmbedder"
    )
    @patch("vectordb.haystack.parent_document_retrieval.search.qdrant.QdrantVectorDB")
    @patch(
        "vectordb.haystack.parent_document_retrieval.search.qdrant.AutoMergingRetriever"
    )
    def test_search_init_loads_config(
        ...
        from vectordb.haystack.parent_document_retrieval.search.qdrant import (
            QdrantParentDocSearchPipeline,
        )

        mock_load_config.return_value = qdrant_config

        pipeline = QdrantParentDocSearchPipeline("config.yaml", sample_parent_store)
    ...
    def test_search_initializes_auto_merger_with_threshold(
        ...
        QdrantParentDocSearchPipeline("config.yaml", sample_parent_store)
