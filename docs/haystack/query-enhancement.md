...
All are exported from `vectordb.haystack.query_enhancement`.
...
2. **Initialize LLM**: For query generation
3. **Generate queries**: `llm.generate(f"Generate 5 variations of: {query}")`
4. **Parse queries**: Extract individual query strings
5. **Embed each query**: `text_embedder.run(text=q)` for each
6. **Retrieve per query**: `db.search(query_embedding, top_k)` for each
7. **Fuse results**: `rrf_fusion_many(result_lists)` for deduplication
8. **Return**: Top-k fused results

### HyDE (Hypothetical Document Embeddings) Flow

1. **Generate hypothetical doc**: `llm.generate(f"Write a document answering: {query}")`
2. **Embed hypothetical doc**: `document_embedder.run(text=hypothetical_doc)`
...
```python
def rrf_fusion_many(result_lists, k=60):
...
```python
from vectordb.haystack.query_enhancement import MilvusQueryEnhancementSearchPipeline
...
```python
from vectordb.haystack.query_enhancement import QdrantQueryEnhancementSearchPipeline
...
```python
from vectordb.haystack.query_enhancement import PineconeQueryEnhancementSearchPipeline
...
```python
from vectordb.haystack.query_enhancement import ChromaQueryEnhancementSearchPipeline
...
| File | Purpose |
|------|---------|
| `src/vectordb/haystack/query_enhancement/__init__.py` | Public API exports |
| `src/vectordb/haystack/query_enhancement/README.md` | Feature overview |
...
| File | Coverage |
|------|----------|
| `tests/haystack/query_enhancement/test_base.py` | Base pipeline tests |
| `tests/haystack/query_enhancement/test_fusion.py` | RRF fusion tests |
| `tests/haystack/query_enhancement/test_*.py` | Per-backend integration tests |

---

**Related Documentation**:

- **Semantic Search** (`docs/haystack/semantic-search.md`): Standard retrieval baseline
- **Reranking** (`docs/haystack/reranking.md`): Post-retrieval scoring
- **Contextual Compression** (`docs/haystack/contextual-compression.md`): Result compression
