# Haystack: Parent Document Retrieval
...
All are exported from `vectordb.haystack.parent_document_retrieval`.
...
1. **Load documents**: Via `DataloaderCatalog.create(...).load().to_haystack()`
...
5. **Embed child chunks**: `document_embedder.run(documents=child_docs)`
...
1. **Embed query**: `text_embedder.run(text=query)`
...
```python
from vectordb.haystack.parent_document_retrieval.utils.types import (
...
```python
from vectordb.haystack.parent_document_retrieval.utils.parent_store import (
...
config_path = "src/vectordb/haystack/parent_document_retrieval/configs/chroma_triviaqa.yaml"
...
for doc in results["documents"]:
    print(f"Parent ({len(doc.content)} chars): {doc.content[:100]}...")
```
...
| File | Purpose |
|------|---------|
| `src/vectordb/haystack/parent_document_retrieval/__init__.py` | Public API exports |
| `src/vectordb/haystack/parent_document_retrieval/README.md` | Feature overview |
...
- **Semantic Search** (`docs/haystack/semantic-search.md`): Standard chunk retrieval
