...
All are exported from `vectordb.haystack.reranking`.
...
5. **Embed query**: `text_embedder.run(text=query)`
6. **Retrieve candidates**: `top_k_candidates` (default 3x final `top_k`)
7. **Load reranker**: Cross-encoder model
8. **Rerank**: `reranker.run(query=query, documents=candidates)`
...
reranked = reranker.run(query=query, documents=candidates, top_k=10)

# 3. Return sorted results
results = sorted(reranked, key=lambda x: x.score, reverse=True)[:top_k]
```
...
indexer = MilvusRerankingIndexingPipeline(
    "src/vectordb/haystack/reranking/configs/milvus_triviaqa.yaml"
)
stats = indexer.run()
...
```python
from vectordb.haystack.reranking import QdrantRerankingSearchPipeline
...
```python
from vectordb.haystack.reranking import PineconeRerankingSearchPipeline
...
from vectordb.haystack.reranking import MilvusRerankingSearchPipeline
...
# Compare
print("Semantic search top result:", semantic_results["documents"][0].content[:100])
print("Reranked search top result:", rerank_results["documents"][0].content[:100])
```
...
| File | Purpose |
|------|---------|
| `src/vectordb/haystack/reranking/__init__.py` | Public API exports |
| `src/vectordb/haystack/reranking/README.md` | Feature overview |
...
| File | Coverage |
|------|----------|
| `tests/haystack/reranking/test_indexing.py` | Indexing pipeline tests |
| `tests/haystack/reranking/test_search.py` | Search pipeline tests |
| `tests/haystack/reranking/test_*.py` | Per-backend integration tests |

### Shared Utilities

| File | Purpose |
|------|---------|
| `src/vectordb/haystack/utils/reranker.py` | Reranker factory |
...
- **MMR** (`docs/haystack/mmr.md`): Diversity-aware reranking
- **Semantic Search** (`docs/haystack/semantic-search.md`): First-stage retrieval
