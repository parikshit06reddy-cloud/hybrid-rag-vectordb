...
This module implements **five backend-specific pipeline pairs** (not one generic class):
...
Each pipeline has the same public shape (`__init__(config_or_path)`, then `run()` / `search()`) but uses backend-specific DB wrapper calls and fusion behavior.
...
- **Haystack**: Supports dense and sparse embedders as separate components
...
2. **Build dense embedder**: `EmbedderFactory.create_document_embedder(config)`
...
5. **Load documents**: `DataloaderCatalog.create(type, split, limit)` → `loader.load().to_haystack()`
...
2. **Build dense query embedder**: `EmbedderFactory.create_text_embedder(config)`
3. **Build sparse query embedder** (optional): Only if top-level `"sparse"` exists
4. **Embed query**: `text_embedder.run(text=query)` → `result["embedding"]`
...
embeddings:
  model: "sentence-transformers/all-MiniLM-L6-v2"
  sparse_model: "prithvida/Splade_v2_Distilbert_uncased"  # Ignored!
```
...
```python
from vectordb.haystack.hybrid_indexing.search.chroma import ChromaHybridSearchPipeline

searcher = ChromaHybridSearchPipeline(
    "src/vectordb/haystack/hybrid_indexing/configs/chroma_triviaqa.yaml"
)
...
```python
from vectordb.haystack.hybrid_indexing import WeaviateHybridSearchPipeline

searcher = WeaviateHybridSearchPipeline(
    "src/vectordb/haystack/hybrid_indexing/configs/weaviate_triviaqa.yaml"
)
...
```python
from vectordb.haystack.hybrid_indexing import PineconeHybridSearchPipeline
...
```python
from vectordb.haystack.hybrid_indexing import QdrantHybridIndexingPipeline

# Recreate collection (drops existing data)
indexer = QdrantHybridIndexingPipeline(
    "src/vectordb/haystack/hybrid_indexing/configs/qdrant_triviaqa.yaml"
)
index_stats = indexer.run()
```
...
```python
from vectordb.haystack.hybrid_indexing import ChromaHybridSearchPipeline
...
| File | Purpose |
|------|---------|
| `src/vectordb/haystack/hybrid_indexing/__init__.py` | Main module exports |
| `src/vectordb/haystack/hybrid_indexing/README.md` | Feature overview |
...
- **Semantic Search** (`docs/haystack/semantic-search.md`): Dense-only retrieval baseline
- **Sparse Indexing** (`docs/haystack/sparse-indexing.md`): Sparse-only retrieval
- **MMR** (`docs/haystack/mmr.md`): Diversity-aware reranking (alternative to fusion)
