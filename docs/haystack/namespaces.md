...
All are exported from `vectordb.haystack.namespaces` along with shared types (`IsolationStrategy`, `TenantStatus`, `NamespaceConfig`, etc.).
...
6. **Embed Documents**: Batch embedding with `SentenceTransformersDocumentEmbedder`
...
6. **Convert to Documents**: Backend response → Haystack `Document` objects
...
```python
from vectordb.haystack.namespaces import IsolationStrategy
...
```python
from vectordb.haystack.namespaces import PineconeNamespacePipeline

pipeline = PineconeNamespacePipeline(
    "src/vectordb/haystack/namespaces/configs/pinecone_config.yaml"
)
...
```python
from vectordb.haystack.namespaces import MilvusNamespacePipeline

pipeline = MilvusNamespacePipeline(
    "src/vectordb/haystack/namespaces/configs/milvus_config.yaml"
)
...
```python
from vectordb.haystack.namespaces import QdrantNamespacePipeline

pipeline = QdrantNamespacePipeline(
    "src/vectordb/haystack/namespaces/configs/qdrant_config.yaml"
)
...
```python
from vectordb.haystack.namespaces import WeaviateNamespacePipeline

pipeline = WeaviateNamespacePipeline(
    "src/vectordb/haystack/namespaces/configs/weaviate_config.yaml"
)
...
```python
from vectordb.haystack.namespaces import ChromaNamespacePipeline

pipeline = ChromaNamespacePipeline(
    "src/vectordb/haystack/namespaces/configs/chroma_config.yaml"
)
...
| File | Purpose |
|------|---------|
| `src/vectordb/haystack/namespaces/__init__.py` | Public API exports |
| `src/vectordb/haystack/namespaces/README.md` | Feature overview |
...
| File | Coverage |
|------|----------|
| `tests/haystack/namespaces/test_types.py` | Type definitions |
| `tests/haystack/namespaces/test_pinecone.py` | Pinecone tests |
| `tests/haystack/namespaces/test_weaviate.py` | Weaviate tests |
| `tests/haystack/namespaces/test_milvus.py` | Milvus tests |
| `tests/haystack/namespaces/test_qdrant.py` | Qdrant tests |
| `tests/haystack/namespaces/test_chroma.py` | Chroma tests |

---

**Related Documentation**:

- **Multi-Tenancy** (`docs/haystack/multi-tenancy.md`): Tenant isolation patterns
