...
|---------|-------------------|-----------------|
| **Chroma** | `ChromaCostOptimizedIndexingPipeline` | `ChromaCostOptimizedSearchPipeline` |
| **Milvus** | `MilvusCostOptimizedIndexingPipeline` | `MilvusCostOptimizedSearchPipeline` |
| **Pinecone** | `PineconeCostOptimizedIndexingPipeline` | `PineconeCostOptimizedSearchPipeline` |
| **Qdrant** | `QdrantCostOptimizedIndexingPipeline` | `QdrantCostOptimizedSearchPipeline` |
| **Weaviate** | `WeaviateCostOptimizedIndexingPipeline` | `WeaviateCostOptimizedSearchPipeline` |

All use typed YAML config with env-var resolution, LangChain `Document` objects, HuggingFace embedders, and ChatGroq generators.
...
| **Vector DB query** | Query latency, payload transfer | `top_k` control, over-fetch only when needed |
...
- **Local embedding**: No per-call API charges (HuggingFace models)
- **Search result capping**: Configurable `top_k`
- **Over-fetch control**: Only when reranking enabled
- **Generation limits**: `generator.max_tokens` caps output
- **Output compaction**: `include_embeddings=False` in results
...
1. **`__init__(config_path)`**:
   - Calls `ConfigLoader.load()` (validation + env var expansion)
   - Creates logger
   - Initializes embeddings via `EmbedderHelper` using `embeddings.model` and `embeddings.batch_size`
   - Calls `warm_up()`
...
2. **`run()`**:
   ...
   - Converts loaded data with `.to_langchain()`
   - Embeds all documents with `embedder.embed_documents(documents=...)`
...
1. **`__init__(config_path)`**:
   - `ConfigLoader.load()` and logger creation
   - `_connect()` to backend
   - `_init_embedder()` with `EmbedderHelper` + `warm_up()`
...
2. **`search(query, top_k=None)`**:
   ...
   - Builds query embedding with `embedder.embed_query(query)`
   - Uses backend query with `search_top_k = top_k * 2` when reranker exists; otherwise `top_k`
   - Converts backend responses to LangChain `Document` objects
   - If reranker exists, calls `reranker.rerank(query=query, documents=...)` and truncates to `top_k`
...
3. **`search_with_rag(query, top_k=None)`**:
   ...
   - If `rag_pipeline` exists and docs present, runs:
     - Prompt formatting with retrieved docs
     - `ChatGroq` generation with `generation_kwargs={temperature, max_tokens}`
...
- **API-based generation**: ChatGroq or OpenAI-compatible endpoints
...
```python
from vectordb.langchain.cost_optimized_rag.base.config import (
    ConfigLoader,  # ${VAR} and ${VAR:-default} support
    RAGConfig,     # Typed model with nested sections
)
from vectordb.langchain.cost_optimized_rag.base.fusion import (
    ResultFuser,  # reciprocal_rank_fusion, weighted_fusion, merge_search_results
)
from vectordb.langchain.cost_optimized_rag.base.metrics import (
...
```python
from vectordb.langchain.cost_optimized_rag.indexing.qdrant_indexer import QdrantIndexingPipeline
from vectordb.langchain.cost_optimized_rag.indexing.pinecone_indexer import PineconeIndexingPipeline
from vectordb.langchain.cost_optimized_rag.indexing.milvus_indexer import MilvusIndexingPipeline
from vectordb.langchain.cost_optimized_rag.indexing.weaviate_indexer import WeaviateIndexingPipeline
from vectordb.langchain.cost_optimized_rag.indexing.chroma_indexer import ChromaIndexingPipeline
```

**Search**:

```python
from vectordb.langchain.cost_optimized_rag.search.qdrant_searcher import QdrantSearchPipeline
from vectordb.langchain.cost_optimized_rag.search.pinecone_searcher import PineconeSearchPipeline
from vectordb.langchain.cost_optimized_rag.search.milvus_searcher import MilvusSearchPipeline
from vectordb.langchain.cost_optimized_rag.search.weaviate_searcher import WeaviateSearchPipeline
from vectordb.langchain.cost_optimized_rag.search.chroma_searcher import ChromaSearchPipeline
```

### Prompt Templates

```python
from vectordb.langchain.cost_optimized_rag.utils.prompt_templates import (
    RAG_ANSWER_TEMPLATE,
...
config_path = "src/vectordb/langchain/cost_optimized_rag/configs/qdrant/triviaqa.yaml"
...
config_path = "src/vectordb/langchain/cost_optimized_rag/configs/pinecone/triviaqa.yaml"
...
```python
from vectordb.langchain.cost_optimized_rag.evaluation.evaluator import RAGEvaluator
...
| File | Purpose |
|------|---------|
| `src/vectordb/langchain/cost_optimized_rag/__init__.py` | Module exports |

### Configuration and Shared Logic

| File | Purpose |
|------|---------|
| `base/config.py` | `ConfigLoader`, `RAGConfig` typed model |
...
- **Semantic Search** (`docs/langchain/semantic-search.md`): Baseline retrieval
