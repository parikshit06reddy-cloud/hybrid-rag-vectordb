...
1. **Route** a query to one of four tools (`search`, `reflect`, `generate`, `calculate`)
...
All five pipelines use LangChain `Document` objects, HuggingFace embedders, and ChatGroq generator.
...
3. **`load_dataset(...)`**:
   ...
   - Populates: `self.data`, `self.documents` (LangChain `Document` list), `self.ground_truths` (question/answer pairs)
4. **`index_documents()`** on backend pipeline
5. **Every backend** calls `embed_documents()` first:
   - Validates `self.documents` exists
   - Runs `embedder.embed_documents(documents=...)`
...
flowchart TD
    ...
    B --> C{Routing Enabled?}
    C -->|Yes| D[router.route query]
    C -->|No| E[Force search Tool]
    D --> F{Tool Selected?}
    E --> F
    F -->|search| G[_handle_search]
    F -->|reflect| H[_handle_reflect]
    F -->|generate| I[_handle_generate]
    F -->|calculate| J[_handle_calculate]
    G --> K[Generate Answer]
    H --> K
    I --> K
    J --> K
...
4. **Tool selection**:
   - If routing enabled: `self.router.route(query)`
   - Else: Force `search`
5. **Tool execution**:
   - `search` → `_handle_search(query, top_k)`
   - `reflect` → `_handle_reflect(query, answer, context)`
   - `generate` → `_handle_generate(query, context)`
   - `calculate` → `_handle_calculate(query)` (LLM-only)
   - Unknown tool → Fallback to search
6. **Search path details**:
   - Backend `_retrieve(query, top_k)` embeds query using `embedder.embed_query(query)`
   - Executes vector query in backend client
   - Maps hits to LangChain `Document(page_content, metadata, score)`
7. **Answer generation** (`_generate_answer`) for search/reasoning:
   - If no docs: Return `"No relevant documents found."`
   - Builds context from top `retrieval.context_top_k` docs (default `5` in code)
   - Calls `ChatGroq` or generator
...
- **Mixed workloads**: Some questions factual search, some reasoning, some calculation
...
| **Current events** | `search` (web search placeholder) |
| **Math/logic** | `calculate` |
| **Complex reasoning** | `reflect` |
...
| **Orchestration** | `_handle_search`, `_handle_reflect`, `_handle_generate`, `_handle_calculate`, `_generate_answer`, `run` |
...
- Unknown tool falls back to search
...
| **`generator.temperature`** | Appears in YAMLs but not wired into generator call |
...
| **Generation errors** | Search answer generation returns `"Answer generation failed."` | Check API key |
...
```python
from vectordb.langchain.agentic_rag import MilvusAgenticRAGPipeline

pipeline = MilvusAgenticRAGPipeline(
    "src/vectordb/langchain/agentic_rag/configs/milvus_triviaqa.yaml"
)
```
...
chroma = ChromaAgenticRAGPipeline(
    "src/vectordb/langchain/agentic_rag/configs/chroma_arc.yaml"
)
qdrant = QdrantAgenticRAGPipeline(
    "src/vectordb/langchain/agentic_rag/configs/qdrant_popqa.yaml"
)
pinecone = PineconeAgenticRAGPipeline(
    "src/vectordb/langchain/agentic_rag/configs/pinecone_factscore.yaml"
)
weaviate = WeaviateAgenticRAGPipeline(
    "src/vectordb/langchain/agentic_rag/configs/weaviate_triviaqa.yaml"
)
```
...
- **Query Enhancement** (`docs/langchain/query-enhancement.md`): Query transformation before retrieval
- **Components** (`docs/langchain/components.md`): Reusable components including router
