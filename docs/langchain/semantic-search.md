...
Semantic search uses **dense embeddings** to retrieve conceptually similar content beyond keyword overlap. This is the foundational retrieval pattern upon which all other advanced features are built.
...
All are exported from `vectordb.langchain.semantic_search`.
...
All indexing pipelines follow this sequence using LangChain components:

1. **Load config**: Via `ConfigLoader.load()` with env var resolution
2. **Validate sections**: Required: `dataloader`, `embeddings`, `<backend>`
3. **Create document embedder**: `EmbedderHelper.create_embedder()` using `HuggingFaceEmbeddings`
...
8. **Embed documents**: `embedder.embed_documents(documents)`
...
3. **Create text embedder**: `EmbedderHelper.create_embedder()` for queries
4. **Warm up embedder**: Load model into memory
...
8. **Optional RAG**: If `rag.enabled=true`, generate answer via `RAGHelper`
...
pipeline = ChromaSemanticIndexingPipeline(config_path)
...
pipeline = ChromaSemanticSearchPipeline(config_path)
result = pipeline.search(query="...", top_k=10)
# Returns {"documents": [...], "query": "...", "db": "chroma"}
```
...
indexer = MilvusSemanticIndexingPipeline(
    "src/vectordb/langchain/semantic_search/configs/milvus_triviaqa.yaml"
)
stats = indexer.run()
...
config_path = "src/vectordb/langchain/semantic_search/configs/pinecone_triviaqa.yaml"
...
config_path = "src/vectordb/langchain/semantic_search/configs/weaviate_triviaqa.yaml"
...
```python
from vectordb.langchain.semantic_search import MilvusSemanticIndexingPipeline
...
| File | Purpose |
|------|---------|
| `src/vectordb/langchain/semantic_search/__init__.py` | Public API exports |
| `src/vectordb/langchain/semantic_search/README.md` | Feature overview |
...
| File | Coverage |
|------|----------|
| `tests/langchain/semantic_search/test_indexing.py` | Indexing pipeline tests |
| `tests/langchain/semantic_search/test_search.py` | Search pipeline tests |
| `tests/langchain/semantic_search/test_*.py` | Per-backend integration tests |

### Shared Utilities

| File | Purpose |
|------|---------|
| `src/vectordb/langchain/utils/embeddings.py` | `EmbedderHelper` for HuggingFace embeddings |
| `src/vectordb/langchain/utils/config.py` | Config loading |
| `src/vectordb/langchain/utils/rag.py` | RAG helper with ChatGroq |
...
- **Sparse Indexing** (`docs/langchain/sparse-indexing.md`): Lexical-only retrieval
- **Reranking** (`docs/langchain/reranking.md`): Second-stage scoring
