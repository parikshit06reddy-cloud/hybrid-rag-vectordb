...
|-----------|----------------|-------------|
| **maximum_margin_relevance** | LangChain `MMRChain` | MMR-style relevance-diversity balance |
| **greedy_diversity_order** | Custom implementation | Greedy variety across ranked list |
...
3. Loaded dataset converted with `.to_langchain()` to LangChain `Document` objects
4. **EmbedderHelper.create_embedder()** creates embeddings from `embedding.model`, `embedding.batch_size`, `embedding.device`
...
flowchart TD
    ...
    E -->|No| F[Return Empty]
    E -->|Yes| G{Algorithm?}
    G -->|MMR/Greedy| H[DiversityRanker]
...
2. Create and warm `EmbedderHelper` for text embedding
3. Encode query via `embedder.embed_query(query)`
...
6. **Select diversity algorithm**:
   - `maximum_margin_relevance` or `greedy_diversity_order`: Diversity ranker
...
8. If `rag.enabled`:
   - Pick prompt via prompt template
   - Format docs for prompt
   - Build prompt
   - Generate via `ChatGroq`
...
```python
from vectordb.langchain.diversity_filtering.utils.config_loader import (
...
```python
from vectordb.langchain.diversity_filtering.utils.prompts import (
...
```python
from langchain.chains import MMRChain
from vectordb.langchain.diversity_filtering.rankers import ClusteringDiversityRanker
```

**LangChain built-in** for MMR:

- `MMRChain` with `k=4` (fetch factor) and `fetch_k=20` (candidates)
...
- Returns selected docs sorted by original `doc.metadata['score']` descending
...
diversity:
  ...
  mmr_lambda: 0.5  # Validated but not always passed to ranker
...
| Config Key | Issue |
|------------|-------|
| **`diversity.mmr_lambda`** | Validated in config model, but **not always passed** to ranker constructors |
...
```python
from vectordb.langchain.diversity_filtering.pipelines.qdrant_indexing import run_indexing

result = run_indexing(
    "src/vectordb/langchain/diversity_filtering/configs/triviaqa_diversity_config.yaml"
)
print(result)
```
...
```python
from vectordb.langchain.diversity_filtering.pipelines.pinecone_search import run_search

result = run_search(
    "src/vectordb/langchain/diversity_filtering/configs/popqa_diversity_config.yaml",
...
```python
from vectordb.langchain.diversity_filtering.pipelines.chroma_search import ChromaDiversitySearchPipeline

pipeline = ChromaDiversitySearchPipeline(
    "src/vectordb/langchain/diversity_filtering/configs/factscore_diversity_config.yaml"
)
...
diverse = ranker.run(query="example", documents=docs)
```

### Example 5: Algorithm Comparison

```python
from vectordb.langchain.diversity_filtering.pipelines.qdrant_search import run_search
...
| File | Purpose |
|------|---------|
| `src/vectordb/langchain/diversity_filtering/__init__.py` | Public exports |
| `src/vectordb/langchain/diversity_filtering/README.md` | Feature overview |
...
- **MMR** (`docs/langchain/mmr.md`): Maximal Marginal Relevance retrieval
- **Reranking** (`docs/langchain/reranking.md`): Relevance-based reranking
